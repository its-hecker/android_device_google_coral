...
if [ "$sdkver" -ge 26 ]; then
	...
	# coral: recovery-as-boot device, always force SETPATCH=true
	# fastboot boot detection incorrectly overrides SETPATCH to false
	# because androidboot.fastboot=1 is set when using fastboot boot
	SETPATCH=true
	log_print 2 "coral: Forcing SETPATCH=true for recovery-as-boot device"
